...
class AbonnementListener implements EventListenerInterface {
    ...
    {
            $notif = '<img src="/instatux/img/avatar/'.$abonnement->nom_session.'.jpg" alt="image utilisateur" class="img-thumbail"/><a href="/instatux/'.$abonnement->nom_session.'">'.$abonnement->nom_session.'</a><span class="alias_tweet">@'.$abonnement->nom_session.'</span> souhaite s\'abonné
    ...
    {
    $notif = '<img src="/instatux/img/avatar/'.$abonnement->nom_session.'.jpg" alt="image utilisateur" class="img-thumbail"/><a href="/instatux/'.$abonnement->nom_session.'">'.$abonnement->nom_session.'</a><span class="alias_tweet">@'.$abonnement->nom_session.'</span> s\'est abonné';
...

{
    $entity = TableRegistry::get('Notifications');
                ...
                $notif = '<img src="/instatux/img/avatar/'.$data_event['nom_session'].'.jpg" alt="image utilisateur" class="img-thumbail"/><a href="/instatux/'.$data_event['nom_session'].'">'.$data_event['nom_session'].'</a> à accepté votre demande d\'abonnement';
