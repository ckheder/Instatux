 ...
 * countlike.js
 *
 * Gestion des like : ajout / suppression
 *
 */
...
$.ajax({
   ...
   {
      $( 'a[data-username="' + data.authname + '"]' ).remove(); // suppression du portrait
      $('#compteur_like-' + id+'').empty('').prepend(' ' + data.nb_like + ''); // mise à jour du nombre de like sur index.ctp
   }
...
{
  $('#compteur_like-' + id+'').empty('').prepend(' ' + data.nb_like + ''); // mise à jour compteur like index
}    
