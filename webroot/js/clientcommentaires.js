/**
 * clientcommentaire.js
 *
 * Gestion des actions en rapport avec les commentaires couplés avec Node JS
 *
 */
...
// node js gestion des comms
  ...
  var socket = io.connect('http://localhost:8083'); //connection
...
// infinite ajax scroll pour la liste des commentaire
  ...
  // quand on ferme la modale, on vide son contenu et on détruit la session IAS des commentaires
...
//notification
    ...
    socket.emit('delete', {idcomm: idcomm,room: room,idtweet: idtweet}); // évènement serveur indiquant une suppression
        ...
        else if(data == 'suppcommfail'){ //échec suppression de commentaire
...
//notification indiquant que les commentaires sont désactivés
...
//notification indiquant que les commentaires sont activés
 ...
 // fin mise à jour commentaire
    ...
    else
    {
      ...
      // affichage spécifique pour l'auteur du comm
        ...
        var nb_comm = $('.viewnbcomm_' + data.tweet_id +'').text();// on récupère le nombre de commentaire

        nb_comm++; // incrémentation du nombre de commentaire
